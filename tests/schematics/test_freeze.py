"""Rescuing what only the database holds. Uses a tiny in-memory index, never the real corpus."""
import gzip
import json
import sqlite3
...
def test_the_three_ocr_conventions(tmp_path):
    assert freeze.ocr_file("ww", "ocr", "ocr/ww/x.pdf.jsonl.gz", None) == "magazines/ocr/ww/x.pdf.jsonl.gz"
    assert freeze.ocr_file("elektor", "ocr+text", "elektor/ocr/1975-02.jsonl.gz", None) == "elektor/ocr/1975-02.jsonl.gz"
    assert freeze.ocr_file("esp", "ocr_boxes", None, "/private/raw/esp/pdf/ab_p1.pdf") == "ocr_boxes/esp/ab_p1.pdf.jsonl.gz"
    assert freeze.ocr_file("esp", "text", None, "/private/raw/esp/html/cd.htm") is None


def test_freezes_only_what_has_no_ocr_file(db, tmp_path):
    counts = freeze.freeze(db, tmp_path, corpus_root=None)     # None: trust the convention, do not check disk
    assert counts["documents"] == 4
    assert counts["with_ocr"] == 2                              # the magazine and the boxed site page
    assert counts["frozen"] == 2                                # the HTML page and the Elektor article
    assert counts["frozen_pages"] == 2                          # the blank second page is skipped


def test_a_frozen_page_is_readable_as_an_ocr_record(db, tmp_path):
    from parts_index.core import pagesio
    from parts_index.core.parts import extractor

    freeze.freeze(db, tmp_path, corpus_root=None)
    path = tmp_path / freeze.frozen_name("esp", "https://sound.example/projects.htm")
...
def test_page_geometry_survives_when_the_database_had_it(db, tmp_path):
    freeze.freeze(db, tmp_path, corpus_root=None)
    rec = json.loads(gzip.open(tmp_path / freeze.frozen_name(
...
def test_hand_checked_links_are_kept(db, tmp_path):
    import csv
    counts = freeze.freeze(db, tmp_path, corpus_root=None)
...
def test_missing_ocr_file_on_disk_is_frozen_instead(db, tmp_path):
    empty = tmp_path / "nothing_on_disk"        # a corpus root that holds no OCR at all
    empty.mkdir()
    counts = freeze.freeze(db, tmp_path / "out", corpus_root=empty)
    assert counts["with_ocr"] == 0 and counts["ocr_missing"] == 2
    assert counts["frozen"] == 3        # the two text documents plus the magazine, which has text in the DB
